...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = pointX*pointZ + pow(pointY, 2);
            // Задаю векторное поле
            double vz = 0.35 * abs(pointY-15);
            // Я хотел сделать бегущую волну, но не понимаю, как это сделать, я как бы понимаю, что
            // y(t,x) = A*sin(wt-kx), но как это через скорость реализовать я не понимаю. В итоге у меня получилась
            // просто деформация по синусу.
            double vy = 2*sin(100*M_PI - M_PI/25*pointX);
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0.0, vy, vz);
        }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "vector" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("t13_myscalar");

    // Считаем STL
    try {
        gmsh::merge("bandura.stl");
    } catch(...) {
